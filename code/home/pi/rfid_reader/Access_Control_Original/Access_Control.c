...
int main(void){
    ...
    printf("status after ISO14443_3_A_PollCard  : %d \n", status);
    ...
    //3 - LOADING KEY
        ...
        status = Mf_Classic_LoadKey(&cardReader, Auth_KeyA, key_ff, sect);
        //status = Mf_Classic_LoadKey(&cardReader, Auth_KeyB, key_ff, sect);
        }
        */

    //4 - READING 
        status = Mf_Classic_Read_Block(&cardReader, TRUE, 8, dataToRead, Auth_KeyA, 2);
        //for(int i = 0; i < 16; i++) printf("%c", dataToRead[i]);
        for(int i = 0; i < 15; i++) result[i] = dataToRead[i];
        printf("status  : %s \n", result);
    ...
    //6 - READING LOOP
        ...
        for(int sect = 0; sect < 5; sect++){
            for(int block = 0; block < 5; block++){
                status = Mf_Classic_Read_Block(&cardReader, TRUE, block, dataToRead, Auth_KeyA, sect);
                for(int i = 0; i < 15; i++) result[i] = dataToRead[i];
                    printf("| Secteur %d | Block %d | : %s \n", sect, block, result);
